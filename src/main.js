...
Object.assign(msg.zh,{createSuccess:"继承计划草案创建成功",createFailed:"继承计划创建失败",deploySuccess:"继承计划已成功广播",deployFailed:"继承计划部署失败",operationFailed:"操作失败",operationSuccess:"操作已成功广播",exportWill:"导出继承计划操作包",deployNow:"立即部署",close:"关闭",kascovWaiting:"正在等待 Kascov 收录并返回验证结果…",kascovIndexed:"Kascov 已收录这笔交易",kascovPending:"Kascov 暂未收录，可稍后点击链接复查；不影响节点已经接受的广播结果。",openKascov:"在默认浏览器打开 Kascov",portableImported:"继承计划操作包已验证并导入",portableImportFailed:"继承计划操作包导入失败",templateRevision:"模板版本",templateRevisionCurrent:"当前模板",exportLegacyTitle:"已导出旧版模板操作包",exportLegacyText:"这份计划使用历史版本的模板编译并部署。操作包只能在包含同一历史模板的 Kas Will 客户端导入；请把对方客户端升级到最新版本后再导入。",ownerRole:"当前钱包：计划建立人，可签到续期或取回",inheritorRole:"当前钱包：继承人；到期后可触发提取",otherRole:"当前钱包：非建立人；只能在到期后触发提取，资产仍按计划固定地址分配",disconnectedRole:"尚未连接钱包；连接后自动识别建立人或提取权限",packageReady:"操作交易已构建，请核对后签名或广播",claimReady:"提取交易已构建，请核对后广播",legacyWill:"旧版本本地记录",legacyWillText:"这是一条旧版本或不支持的本机记录。可以删除本机记录；不会影响链上资产。",deleteLocalWill:"删除本机记录",deleteWillConfirm:"这只会删除本机工作记录，不会撤回链上资产或改变 Covenant。确定删除？",deleteWillSuccess:"本机记录已删除",deleteWillFailed:"删除失败"});
...
const SERVER_ERROR_TEXT={
  RENEWAL_EXPIRED:{zh:"继承计划已到期，不能再签到续期；到期后只能触发继承分配。",en:"The will has matured and can no longer be renewed; only the mature distribution is available."},
  RENEWAL_ALREADY_SPENT:{zh:"该计划金库已被支出，无法续期。",en:"This will vault has already been spent."},
  INHERITANCE_NOT_MATURE:{zh:"继承计划尚未到期，暂时不能触发继承分配。",en:"The will has not matured yet; distribution is not available."},
  INHERITANCE_ALREADY_SPENT:{zh:"该计划资产已被支出。",en:"This will has already been spent."},
...
function renderProjectSelect(){ const select=$("#operate-project"); if(!select)return; const selected=select.value; const wills=state.projects.filter(isWillProject); select.innerHTML=`<option value="">${state.language==="zh"?"选择计划":"Select a will"}</option>`+wills.map(p=>`<option value="${esc(p.id)}">${esc(p.name)}</option>`).join(""); if(wills.some(p=>p.id===selected))select.value=selected; }
...
async function renderLifecycle(){
  ...
  if(!id){state.activeProject=null;exportButton.disabled=true;deleteButton.disabled=true;renderWalletRole(null);state.countdownTargetMs=null;card.removeAttribute("aria-busy");card.innerHTML=`<div class="empty">${state.language==="zh"?"选择一个计划查看链上状态。":"Select a will to inspect."}</div>`;return;}
  ...
  if(!project){card.removeAttribute("aria-busy");card.innerHTML=`<div class="empty">${state.language==="zh"?"找不到这份计划，请刷新列表。":"This will could not be found. Refresh the list."}</div>`;return;}
  ...
  try{
    ...
    // Arm the ticker only for a live schedule. A mature will resolves to
    // zero remaining seconds, which would otherwise re-trigger a full
    // lifecycle refresh every tick and make the card flash endlessly.
    const liveSchedule=status.schedule&&!status.schedule.mature&&Number(status.schedule.approximateRemainingSeconds)>0;
    state.countdownTargetMs=liveSchedule?Date.now()+status.schedule.approximateRemainingSeconds*1000:null;
...
async function broadcastPackage(){
  ...
  await exclusive(async()=>{
    ...
    try{
      ...
      if(state.activeProject)$("#operate-project").value=state.activeProject.id;
      hideProgress();
      showResult({title:t("operationSuccess"),message:operationId==="inherit"?(state.language==="zh"?"继承分配交易已提交，资产将严格发送到计划固定的继承地址。":"The distribution transaction was submitted; assets remain bound to the fixed inheritor addresses."):t("broadcasted"),details:[{label:"TXID",value:result.txid},{label:"Kascov",value:result.kascovTransactionUrl,href:result.kascovTransactionUrl}],actions:[{label:t("openKascov"),onClick:()=>openUrl(result.kascovTransactionUrl)}]});
